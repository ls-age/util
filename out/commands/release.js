...
function createRelease(opts, releaseAs) {
  return (0, _run2.default)('npm run prepublish').then(() => (0, _run2.default)('git add -f out')).then(() => (0, _run2.default)('git add -f docs/api')).then(() => new Promise((resolve, reject) => {
    (0, _standardVersion2.default)({
      infile: (0, _path.join)(process.cwd(), 'CHANGELOG.md'),
...
function undoRelease(env) {
  return new Promise((resolve, reject) => {
    (0, _fs.readFile)(env.packagePath, 'utf8', (readErr, contents) => {
      if (readErr) {
        reject(readErr);
      } else {
        try {
  ...
  }).then(version => (0, _run2.default)(`git tag -d v${ version }`)) // Delete tag
  .then(() => (0, _run2.default)('git reset --hard HEAD~2')); // Undo release commits
}
...
function run(opts, env) {
  ...
  return (0, _runStep2.default)('Prepare release', this.prepareRelease(opts)).then(() => (0, _runStep2.default)('Create release', createRelease(opts, releaseAs))).then(() => confirmPublish(opts)).then(publish => {
    ...
    return (0, _runStep2.default)('Undo release', undoRelease(env));
  });
}
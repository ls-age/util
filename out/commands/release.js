...
function run(opts, env) {
  ...
  }).then(() => (0, _run2.default)('git push')).then(() => (0, _run2.default)('npm run prepublish')).then(() => (0, _run2.default)('git add -f out')).then(() => (0, _run2.default)('git add -f docs/api')).then(() => new Promise((resolve, reject) => {
  ...
  })).then(() => (0, _run2.default)('git rm -rf out')).then(() => (0, _run2.default)('git rm -rf docs/api')).then(() => (0, _run2.default)(['git', 'commit', '-m', '"chore(release): Remove generated files"'])).then(() => console.log('Now, create github release', opts.createRelease));
}